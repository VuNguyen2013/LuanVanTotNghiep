...
{		
	...
	public partial class ConditionOrderService : ETradeOrders.Services.ConditionOrderServiceBase
	{
	    ...
	    ///<param name="fromTool">Tool calling this method.</param>
	    ///<returns>
	    /// A List&lt;ConditionOrder&gt object contains condition order information.
	    /// </returns>
	    public List<ConditionOrder> GetListConditionOrder(string accountNo, string side, string symbol, int[] status, 
            string fromDate, string toDate, int pageIndex, int pageSize, out int count, int fromTool)
        {
            var whereClause = new StringBuilder();
            if (fromTool == (int) CommonEnums.TOOL_USING.ETRADE_MONITOR)
            {
                whereClause.AppendFormat(
                    "AND (DoneTime is null or DoneTime = " + Constants.SQL_CONVERT_DATETIME_YYYYMMDD + " )",
                ...
                if (conditionOrder != null)
                {
                                ...
                                || (execOrder.OrderStatus == (short)CommonEnums.ORDER_STATUS.SEMI_MATCHED))
                    {
                        ...
                        if (conditionOrder.MatchedVolume == conditionOrder.Volume)
                        {
                            ...
                            conditionOrder.DoneTime = DateTime.Now.Date;
                        }
                    }
                    bool result = Update(conditionOrder);
	    ...
	    ///<returns></returns>
	    public bool UpdateConditionOrderStatus(long conditionOrderId, string status, int rejectReason)
        {
            var conditionOrder = GetByConditionOrderId(conditionOrderId);
            if (conditionOrder != null)
            {
                ...
                if (rejectReason > 0)
                {
                    conditionOrder.RejectReason = rejectReason;
                }
                bool result = Update(conditionOrder);
                if (result)
                {
