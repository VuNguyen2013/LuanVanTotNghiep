...
namespace ETradeGWServices
{
    class EtradeGWCommonUtils
    {
        ...
        static string ServiceID(char serviceName)
        {
            switch (serviceName)
            {
                case (char) CommonEnums.ORDER_SOURCE.FROM_WEB:
                    return "01";
                case (char)CommonEnums.ORDER_SOURCE.FROM_IVR:
                    return "02";
                case (char)CommonEnums.ORDER_SOURCE.FROM_SMS:
                    return "03";
                case (char)CommonEnums.ORDER_SOURCE.FROM_MOBILE:
                    return "03";
                default:
                    return "";
            }
        ...
        static public string GetRefOrderID(long refOrderID, char serviceName)
        {
            string serviceID = ServiceID(serviceName);
            string sRefOrderID = refOrderID.ToString(ETradeGWCommonConstants.ZERO_REFORDERID);
        ...
        static public CommonEnums.ORDER_SOURCE GetOrderSource(string refOrderID)
        {
            ...
            switch (sourceID)
            {
                ...
                case "04":
                    return CommonEnums.ORDER_SOURCE.FROM_MOBILE;
                default:
                    return CommonEnums.ORDER_SOURCE.FROM_IFIS_BROKER;
            }
        }
