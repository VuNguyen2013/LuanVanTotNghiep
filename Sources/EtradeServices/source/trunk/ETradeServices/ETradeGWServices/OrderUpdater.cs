...
using ETradeGWServices.RTServices;
...
namespace ETradeGWServices
{
    ...
    public class OrderUpdater
    {
                ...
                switch (orderInfo.Type)
                {
                    ...
                    case ETradeGWCommonConstants.DATA_CHANGE_ORDER_FROM_BROKER:
                        ...
                        if (execOrder != null)
                        {
                            ...
                            execOrder.Sequence = orderInfo.Sequence;
                            execOrder.IsNewStatus = true;
                            if (orderInfo.Volume > 0)
                                execOrder.Volume = orderInfo.Volume;
                            if (!string.IsNullOrEmpty(orderInfo.Side.ToString().Trim()))
                            {
                                execOrder.Side = orderInfo.Side.ToString().Trim();
                            }
                            execOrder.ConPrice = orderInfo.ConPrice.ToString();
                            if (execOrder.ConPrice == Constants.ORDER_TYPE_ATO.ToString())
                            {
                                execOrder.Price = (int)CommonEnums.COND_PRICE.ATO;
                            }
                            else if (execOrder.ConPrice == Constants.ORDER_TYPE_ATC.ToString())
                            {
                                execOrder.Price = (int)CommonEnums.COND_PRICE.ATC;
                            }
                            else if (execOrder.ConPrice == Constants.ORDER_TYPE_MP.ToString())
                            {
                                execOrder.Price = (int)CommonEnums.COND_PRICE.MP;
                            }
                            else if (execOrder.ConPrice == Constants.ORDER_TYPE_MAK.ToString())
                            {
                                execOrder.Price = (int)CommonEnums.COND_PRICE.MAK;
                            }
                            else if (execOrder.ConPrice == Constants.ORDER_TYPE_MOK.ToString())
                            {
                                execOrder.Price = (int)CommonEnums.COND_PRICE.MOK;
                            }
                            else
                            {
                                execOrder.Price = new decimal(orderInfo.Price);
                ...
                try
                {
                    ...
                    if (AppConfig.CheckGWConnection && !_messageHandler.IsConnected())
                    {
                        ...
                        if (connected)
                        {
                            ...
                            //if (logged)
                            //{
                            //    LogHandler.Log("Connected LinkOPS.", GetType() + ".DataFromFISReceiver()",
                            //           TraceEventType.Information);
                            //}
                            //else
                            //{
                            //    LogHandler.Log("Cannot not connect,Logon LinkOPS", GetType() + ".DataFromFISReceiver()",
                    ...
                    if (_messageHandler.HasOrder())
                    {
                        ...
                        _messageHandler.GetOrder(ref orderInfo.Sequence, ref orderInfo.Time, ref orderInfo.Type, ref orderInfo.RefOrderID,
                                                 ...
                                                 ref orderInfo.Status, ref orderInfo.OrdRejReason, ref orderInfo.ExecTransType, ref orderInfo.SourceID, ref orderInfo.OrdRejText);
                    ...
                    case (short) CommonEnums.ORDER_STATUS.CONFIRMED_FIS:
                        ...
                        switch (orderInfo.Type)
                        {
                            case ETradeGWCommonConstants.DATA_NEW_CANCEL_ACK: //7b
                                ...
                                if(orderInfo.ExecTransType == (short)ETradeGWServices.TRANS_TYPE.TRANS_CHANGE_ACC)
                                {
                                    execOrder.OrderStatus = (short)CommonEnums.ORDER_STATUS.CONFIRMED_SET;
                                    return;
                                }
                            ...
                            case ETradeGWCommonConstants.DATA_EXEC_REPORT: //7e
                                if (orderInfo.Volume > 0)
                                {
                                    if (execOrder.OrderStatus != (short)CommonEnums.ORDER_STATUS.WAITING_CANCEL)
                                    {
                                        execOrder.OrdRejReason = (int) CommonEnums.REJECT_REASON.NO_CORRELATIVE_ORDER;
                                    }
                    ...
                    case (short) CommonEnums.ORDER_STATUS.CONFIRMED_SET:
                        ...
                        switch (orderInfo.Type)
                        {
                            ...
                            case ETradeGWCommonConstants.DATA_EXEC_REPORT://7e
                                ...
                                {
                                    execOrder.OrderStatus = (short) CommonEnums.ORDER_STATUS.SEMI_MATCHED;
                                    if (execOrder.ConPrice == Constants.ORDER_TYPE_MP.ToString())
                                    {
                                        execOrder.ConPrice = Constants.ORDER_TYPE_LO.ToString();
                                        LogHandler.Log("1. Price: "+orderInfo.Price,"GetNewPriceForMpOrder",TraceEventType.Information);
                                        decimal price = GetNewPriceForMpOrder(execOrder, orderInfo);
                                        LogHandler.Log("2. Price: " + price, "GetNewPriceForMpOrder", TraceEventType.Information);
        ...
        private static decimal GetNewPriceForMpOrder(ExecOrder execOrder, OrderInfo orderInfo)
        {
            decimal price =  decimal.Parse(orderInfo.Price.ToString());
            
            Service _rtService =new Service();
            var resultObject =
                Serializer.Deserialize<ResultObject<StockInfo>>(
                    _rtService.GetStockInfo(execOrder.SecSymbol));
            if (resultObject.Result != null)
            {
                decimal celling = decimal.Parse(resultObject.Result.Ceiling.ToString());
                decimal floor = decimal.Parse(resultObject.Result.Floor.ToString());
                short market = resultObject.Result.MarketID;
                if (execOrder.Side == ((char)CommonEnums.TRADE_SIDE.BUY).ToString())
                {
                    switch (market)
                    {
                        case (short) CommonEnums.MARKET_ID.HOSE:
                            if ((price + (decimal) 0.1) >= celling)
                            {
                                price = celling;
                            }
                            else
                            {
                                if (
                                    ETradeCommon.Utils.IsValidStepPrice(
                                        (int) CommonEnums.MARKET_ID.HOSE,
                                        price + (decimal) 0.1, Constants.ORDER_TYPE_LO) !=
                                    CommonEnums.REJECT_REASON.IS_VALID)
                                {
                                    if (
                                        ETradeCommon.Utils.IsValidStepPrice(
                                            (int) CommonEnums.MARKET_ID.HOSE,
                                            price + (decimal) 0.5, Constants.ORDER_TYPE_LO) !=
                                        CommonEnums.REJECT_REASON.IS_VALID)
                                    {
                                        if (
                                            ETradeCommon.Utils.IsValidStepPrice(
                                                (int) CommonEnums.MARKET_ID.HOSE, price + 1,
                                                Constants.ORDER_TYPE_LO) ==
                                            CommonEnums.REJECT_REASON.IS_VALID)
                                        {
                                            price = price + 1;
                                        }
                                        ...
                                        {
                                            LogHandler.Log(
                                                "IsValidStepPrice: incorrect step price: MP order change to LO order, price = " +
                                                price +
                                                ", market = " +
                                                market + ", code: " +
                                                execOrder.SecSymbol,
                        ...
                        case (short) CommonEnums.MARKET_ID.UPCoM:
                            ...
                            else
                            {
                                if (
                                    ETradeCommon.Utils.IsValidStepPrice(
                                        (int) CommonEnums.MARKET_ID.UPCoM,
                                        price + (decimal) 0.1, Constants.ORDER_TYPE_LO) !=
                                    CommonEnums.REJECT_REASON.IS_VALID)
                                {
                                    LogHandler.Log(
                        ...
                        default:
                            LogHandler.Log(
                                "IsValidMarket: Error parse market: " +
                                execOrder.Market,
                                "SetExecOrderValues.",
                                TraceEventType.Warning);
                            break;
                ...
                else
                {
                    ...
                    {
                        case (short) CommonEnums.MARKET_ID.HOSE:
                            if (((price - (decimal) 0.1) <= floor) ||
                                ((price - (decimal) 0.5) <= floor) ||
                                ((price - 1) <= floor))
                            {
                                price = floor;
                            }
