...
namespace ETradeWebServices.Services
{
    ...
    public static class MarketServices
    {
        ...
        //private static char _tradingStatusHOSE = (char)CommonEnums.MARKET_STATUS.OPEN;//change to test
        //private static char _marketStatusHOSE = (char)CommonEnums.MARKET_STATUS.OPEN;//change to test
        ...
        //private static char _tradingStatusHNX = (char)CommonEnums.MARKET_STATUS.OPEN;//change to test
        //private static char _marketStatusHNX = (char)CommonEnums.MARKET_STATUS.OPEN;//chang to test
        ...
        //private static char _tradingStatusUpcom = (char)CommonEnums.MARKET_STATUS.OPEN;//change to test
        //private static char _marketStatusUpcom = (char)CommonEnums.MARKET_STATUS.OPEN;//chang to test
        ...
        private static void UpdateSessionHOSE(char marketState, DateTime sessionTime)
        {
            ...
            switch (_tradingStatusHOSE)
            {
                ...
                case (char)CommonEnums.MARKET_STATUS.PRE_OPEN:
                    {
                        ...
                        if ((duration.TotalSeconds > AppConfig.PreOpenDurationHose) ||
                ...
                case (char)CommonEnums.MARKET_STATUS.OPEN:
                    {
                        ...
                        if (AppConfig.AfternoonOpenDurationHose == 0)
                        {
                            if ((duration.TotalSeconds > AppConfig.MorningOpenDurationHose) ||
                ...
                case (char)CommonEnums.MARKET_STATUS.OPEN_2:
                    {
                        TimeSpan duration = sessionTime - _sessionTimeHOSE;
                        if ((duration.TotalSeconds > AppConfig.AfternoonOpenDurationHose) ||
                            (marketState == (char) CommonEnums.MARKET_STATUS.PRE_CLOSE))
                        {
                            _tradingStatusHOSE = (char) CommonEnums.MARKET_STATUS.PRE_CLOSE;
                            _sessionTimeHOSE = sessionTime;
                ...
                case (char)CommonEnums.MARKET_STATUS.PRE_CLOSE:
                    {
                        TimeSpan duration = sessionTime - _sessionTimeHOSE;
                        if (duration.TotalSeconds > AppConfig.PreCloseDurationHose ||
        ...
        private static void UpdateSessionHNX(char marketState, DateTime sessionTime)
        {
            ...
            switch (_tradingStatusHNX)
            {
                ...
                case (char)CommonEnums.MARKET_STATUS.UNVAILABLE:
                    _tradingStatusHNX = marketState;
                    _sessionTimeHNX = sessionTime;
                    if (_tradingStatusHNX == (char)CommonEnums.MARKET_STATUS.PRE_OPEN || _tradingStatusHNX == (char)CommonEnums.MARKET_STATUS.OPEN)
                    {
                    ...
                    break;
                case (char)CommonEnums.MARKET_STATUS.READY:
                    if (marketState == (char)CommonEnums.MARKET_STATUS.PRE_OPEN || marketState == (char)CommonEnums.MARKET_STATUS.OPEN)
                    {
                        _tradingStatusHNX = marketState;
                        _sessionTimeHNX = sessionTime;
                        HandleHNXCheckConditionOrderEvent(null, EventArgs.Empty);
                    ...
                    break;
                case (char)CommonEnums.MARKET_STATUS.PRE_OPEN://sau nay co phien ATO se dung lai doan code nay
                    {
                        TimeSpan duration = sessionTime - _sessionTimeHNX;
                        if ((duration.TotalSeconds > AppConfig.PreOpenDurationHnx) ||
                            marketState == (char)CommonEnums.MARKET_STATUS.OPEN)
                        {
                            _tradingStatusHNX = (char)CommonEnums.MARKET_STATUS.OPEN;
                            _sessionTimeHNX = sessionTime;
                            HandleHNXCheckConditionOrderEvent(null, EventArgs.Empty);

                        }
                        else if(marketState == (char)CommonEnums.MARKET_STATUS.OPEN_2)//truong hop nay chi xay ra khi TEST FISDB
                        {
                            _tradingStatusHNX = (char)CommonEnums.MARKET_STATUS.OPEN_2;
                            _sessionTimeHNX = sessionTime;
                        }
                    }
                    break;
                case (char)CommonEnums.MARKET_STATUS.OPEN:
                    {
                        ...
                        //support for trading only on morning.
                        if (AppConfig.AfternoonOpenDurationHnx == 0)
                        {
                            if ((duration.TotalSeconds > AppConfig.MorningOpenDurationHnx) ||
                                (marketState == (char)CommonEnums.MARKET_STATUS.PRE_CLOSE))
                            {
                                _tradingStatusHNX = (char)CommonEnums.MARKET_STATUS.PRE_CLOSE;
                                _sessionTimeHNX = sessionTime;
                                HandleHNXCheckConditionOrderEvent(null, EventArgs.Empty);
                                return;
                            }
                ...
                case (char)CommonEnums.MARKET_STATUS.OPEN_2:
                    {
                        TimeSpan duration = sessionTime - _sessionTimeHNX;
                        if ((duration.TotalSeconds > AppConfig.AfternoonOpenDurationHnx) ||
                            (marketState == (char)CommonEnums.MARKET_STATUS.PRE_CLOSE))
                        {
                            _tradingStatusHNX = (char)CommonEnums.MARKET_STATUS.PRE_CLOSE;
                            _sessionTimeHNX = sessionTime;
                ...
                case (char)CommonEnums.MARKET_STATUS.PRE_CLOSE:
                    {
                        if (marketState == (char)CommonEnums.MARKET_STATUS.PRE_CLOSE2)
                        {
                            _tradingStatusHNX = marketState;
                            _sessionTimeHNX = sessionTime;
                        }
                        else if (marketState == (char)CommonEnums.MARKET_STATUS.CLOSE)
                        {
                            TimeSpan duration = sessionTime - _sessionTimeHNX;
                            if (duration.TotalSeconds > AppConfig.PreCloseDurationHnx)
                            {
                    ...
                    break;
                case (char)CommonEnums.MARKET_STATUS.PRE_CLOSE2:
                    {
                        TimeSpan duration = sessionTime - _sessionTimeHNX;
                        if (duration.TotalSeconds > AppConfig.PreCloseDurationHnx ||
                            marketState == (char)CommonEnums.MARKET_STATUS.CLOSE)
                ...
                case (char)CommonEnums.MARKET_STATUS.CLOSE:
                    {
                        if (marketState == (char)CommonEnums.MARKET_STATUS.AFTER_CLOSE)
                        {
                            _tradingStatusHNX = marketState;
                            _sessionTimeHNX = sessionTime;
                        }
                        else if (marketState == (char)CommonEnums.MARKET_STATUS.CLOSE_PT)
                        {
                            _tradingStatusHNX = marketState;
                            _sessionTimeHNX = sessionTime;
                        }
                    ...
                    break;
                case (char)CommonEnums.MARKET_STATUS.AFTER_CLOSE:
                    {
                        if (marketState == (char)CommonEnums.MARKET_STATUS.CLOSE_PT)
                        {
                            _tradingStatusHNX = marketState;
                ...
                default:
                    _tradingStatusHNX = marketState;
                    _sessionTimeHNX = sessionTime;
                    if (_tradingStatusHNX == (char)CommonEnums.MARKET_STATUS.READY)
                    {
                        HandleHNXCheckConditionOrderEvent(null, EventArgs.Empty);
        ...
        private static void UpdateSessionUpcom(char marketState, DateTime sessionTime)
        {
            ...
            switch (_tradingStatusUpcom)
            {
                ...
                case (char)CommonEnums.MARKET_STATUS.UNVAILABLE:
                    _tradingStatusUpcom = marketState;
                    _sessionTimeUpcom = sessionTime;
                    if (marketState == (char)CommonEnums.MARKET_STATUS.PRE_OPEN || _tradingStatusUpcom == (char)CommonEnums.MARKET_STATUS.OPEN)
                    {
                ...
                case (char)CommonEnums.MARKET_STATUS.PRE_OPEN:
                    {
                        ...
                        if ((duration.TotalSeconds > AppConfig.PreOpenDurationHnx) ||
                            marketState == (char)CommonEnums.MARKET_STATUS.OPEN)
                        {
                            _tradingStatusUpcom = (char)CommonEnums.MARKET_STATUS.OPEN;
                            _sessionTimeUpcom = sessionTime;
                            HandleUPCOMCheckConditionOrderEvent(null, EventArgs.Empty);
                        ...
                        {
                            _tradingStatusUpcom = (char)CommonEnums.MARKET_STATUS.OPEN_2;
                            _sessionTimeUpcom = sessionTime;
                        }
                    }
                    break;
                case (char)CommonEnums.MARKET_STATUS.OPEN:
                    {
                        ...
                        if (AppConfig.AfternoonOpenDurationHnx == 0)
                        {
                            if ((duration.TotalSeconds > AppConfig.MorningOpenDurationHnx) ||
                                (marketState == (char)CommonEnums.MARKET_STATUS.PRE_CLOSE))
                            {
                                _tradingStatusUpcom = (char)CommonEnums.MARKET_STATUS.PRE_CLOSE;
                                _sessionTimeUpcom = sessionTime;
                                HandleUPCOMCheckConditionOrderEvent(null, EventArgs.Empty);
                                return;
                            }
                ...
                case (char)CommonEnums.MARKET_STATUS.CLOSE:
                    {
                        if (marketState == (char)CommonEnums.MARKET_STATUS.AFTER_CLOSE ||
                            marketState == (char)CommonEnums.MARKET_STATUS.CLOSE_PT)
                        {
                            _tradingStatusUpcom = marketState;
                            _sessionTimeUpcom = sessionTime;
                        }
                ...
                default:
                    _tradingStatusUpcom = marketState;
                    _sessionTimeUpcom = sessionTime;
                    if (_tradingStatusUpcom == (char)CommonEnums.MARKET_STATUS.READY)
                    {
                        HandleUPCOMCheckConditionOrderEvent(null, EventArgs.Empty);
                    }
                    break;
