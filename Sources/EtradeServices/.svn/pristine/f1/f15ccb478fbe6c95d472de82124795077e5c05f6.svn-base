...
{		
	...
	public partial class FeeService : ETradeFinance.Services.FeeServiceBase
	{
        ...
        public decimal SelValueAfterFee(decimal sellValue)
        {
            //Caculate the fee.
            Fee fee = GetTradeFee(CommonEnums.FEE_TYPE.FEE_TRADE, sellValue);

            if (fee != null)
            {
                return Constants.PERCENT_UNIT!=0? (decimal)(sellValue * (1 - fee.FeeRatio / Constants.PERCENT_UNIT - fee.Vat / Constants.PERCENT_UNIT)):0;
            }

            return sellValue;
        }

        public decimal BuyValueAfterFee(decimal buyValue)
        {
            //Caculate the fee.
            Fee fee = GetTradeFee(CommonEnums.FEE_TYPE.FEE_TRADE, buyValue);

            if (fee != null)
            {
                return Constants.PERCENT_UNIT!=0? (decimal)(buyValue * (1 + fee.FeeRatio / Constants.PERCENT_UNIT)):0;
            }
